...
float _qRF0_4x1[5];
float _qLF0_4x1[5];
float _RHY0, _RHR0, _RHP0, _RKN0, _RAP0, _RAR0, _LHY0, _LHR0, _LHP0, _LKN0, _LAP0, _LAR0;
float _WST0, _RSP0, _RSR0, _RSY0, _REB0, _RWY0, _RWP0, _LSP0, _LSR0, _LSY0, _LEB0, _LWY0, _LWP0;
//---------------------
...
*/
	...
	//------------------ initial position where joints return to,
	_RHY0 = _Qlb_19x1[8];
	_RHR0 = _Qlb_19x1[9];
	_RHP0 = _Qlb_19x1[10];
	_RKN0 = _Qlb_19x1[11];
...
int LoadMocapData2(int MocapNo)	// for preview window
{
	...
	float ftemp;
	FILE *fp;
	float scale_WST, scale_RSP, scale_RSR, scale_RSY, scale_REP, scale_REY, scale_LSP, scale_LSR, scale_LSY, scale_LEP, scale_LEY;

	switch(MocapNo)
	{
	case 1:
		if ( (fp =fopen( "C:\\Rainbow\\MotionCapture\\mocap_converted_1.txt","r")) == NULL )
		{
			wberror("Data file mocap_converted_1.txt not found\n");		
			return 1;
		}
		break;
	case 2:
		if ( (fp =fopen( "C:\\Rainbow\\MotionCapture\\mocap_converted_2.txt","r")) == NULL )
		{
			wberror("Data file mocap_converted_2.txt not found\n");		
			return 1;
		}
		break;
	default:
	...
	for (k=1; k<=_MOTION_LENGTH; k++)
	{
		fscanf(fp,"%f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f"
			,&_des_PCz[k], &_des_qPEL[k][1], &_des_qPEL[k][2], &_des_qPEL[k][3], &_des_qPEL[k][4]
			,&_des_WST[k], &_des_RSP[k], &_des_RSR[k], &_des_RSY[k], &_des_REP[k], &_des_REY[k]
			,&_des_LSP[k], &_des_LSR[k], &_des_LSY[k], &_des_LEP[k], &_des_LEY[k]);		

		_des_WST[k] *= scale_WST;
		_des_RSP[k] *= scale_RSP;
		_des_RSR[k] *= scale_RSR;
		_des_RSY[k] *= scale_RSY;
		_des_REP[k] *= scale_REP;
		_des_REY[k] *= scale_REY;

		_des_LSP[k] *= scale_LSP;
		_des_LSR[k] *= scale_LSR;
		_des_LSY[k] *= scale_LSY;
		_des_LEP[k] *= scale_LEP;
		_des_LEY[k] *= scale_LEY;
	}
...
int MocapOnestep(unsigned int n)
{
	...
	const float des_pRFC_3x1[4] = {0., _pRFC0_3x1[1], _pRFC0_3x1[2], _pRFC0_3x1[3]};
	const float des_pLFC_3x1[4] = {0., _pLFC0_3x1[1], _pLFC0_3x1[2], _pLFC0_3x1[3]};
	const float des_qRF_4x1[5] = {0., _qRF0_4x1[1], _qRF0_4x1[2], _qRF0_4x1[3], _qRF0_4x1[4]};
	const float des_qLF_4x1[5] = {0., _qLF0_4x1[1], _qLF0_4x1[2], _qLF0_4x1[3], _qLF0_4x1[4]};
	...
	QTdel(des_qRF_4x1, qRF_4x1, temp4_3x1);		
	...
	QTdel(des_qLF_4x1, qLF_4x1, temp4_3x1);
...
int UpdatePassiveCoord_SSP(int LSP_0_RSP_1)
{
	...
	float temp2_12x1[13], temp3_3x1[4];
	...
	if(LSP_0_RSP_1 == 0)	// Left foot supporting
	{
		mult_mm((const float**)_dcPEL_3x3,3,3, (const float**)_Rz_LHY_3x3,3, _TEMP1_18x18);
		mult_mm((const float**)_Rx_LHR_3x3,3,3, (const float**)_Ry_LHP_3x3,3, _TEMP2_18x18);
		mult_mm((const float**)_TEMP1_18x18,3,3, (const float**)_TEMP2_18x18,3, _dcLUL_3x3);
		...
		trans(1.f, (const float**)_dcLF_3x3,3,3, _dcPEL_3x3);
		
	}
	else		// Right foot supporting
	{
		mult_mm((const float**)_dcPEL_3x3,3,3, (const float**)_Rz_RHY_3x3,3, _TEMP1_18x18);
		mult_mm((const float**)_Rx_RHR_3x3,3,3, (const float**)_Ry_RHP_3x3,3, _TEMP2_18x18);
		mult_mm((const float**)_TEMP1_18x18,3,3, (const float**)_TEMP2_18x18,3, _dcRUL_3x3);
		...
		trans(1.f, (const float**)_dcRF_3x3,3,3, _dcPEL_3x3);		
	}		
	...
	DC2QT((const float**)_dcPEL_3x3, temp2_12x1);
	_Qlb_19x1[4] = temp2_12x1[1];
	_Qlb_19x1[5] = temp2_12x1[2];
	_Qlb_19x1[6] = temp2_12x1[3];
	_Qlb_19x1[7] = temp2_12x1[4];	

	_pLFC0_3x1[1] = pLFC[1];
	_pLFC0_3x1[2] = pLFC[2];
	_pLFC0_3x1[3] = pLFC[3] + _Qlb_19x1[3];

	_pRFC0_3x1[1] = pRFC[1];
	_pRFC0_3x1[2] = pRFC[2];
	_pRFC0_3x1[3] = pRFC[3] + _Qlb_19x1[3];

	_pPCz0 = _Qlb_19x1[3];

	DC2QT((const float**)_dcLF_3x3, _qLF0_4x1);
	DC2QT((const float**)_dcRF_3x3, _qRF0_4x1);
...
int PushQubWindow(float qPEL0, float qPELx, float qPELy, float qPELz, float PCz, 
					 ...
					 float body_pitch)
{
	unsigned int head_new_qPEL = (_Qub_window.head_qPEL+1)%WIND_SIZE_QPEL; 
	unsigned int head_new_PCz = (_Qub_window.head_PCz+1)%WIND_SIZE_PCZ; 
	...
	_Qub_window.wind_qPEL[head_new_qPEL][1] = qPEL0;
	_Qub_window.wind_qPEL[head_new_qPEL][2] = qPELx;
	_Qub_window.wind_qPEL[head_new_qPEL][3] = qPELy;
	_Qub_window.wind_qPEL[head_new_qPEL][4] = qPELz;
	_Qub_window.wind_PCz[head_new_PCz] = PCz;
	_Qub_window.wind_WST[head_new_WST] = wst;
	_Qub_window.wind_RSP[head_new_RSP] = rsp;
	_Qub_window.wind_RSR[head_new_RSR] = rsr;
	...
	_Qub_window.wind_LSR[head_new_LSR] = lsr;
	...
	_Qub_window.head_qPEL = head_new_qPEL;
	_Qub_window.head_PCz = head_new_PCz;
...
int MocapOnestep_JW(void)
{
	...
	if(_Qubp_11x1[1] < 0.)
	{
		if((wstmin < _Qub_window.minWST) && (_Qub_11x1[1] >_Qub_window.minWST))
			wstmin = _Qub_window.minWST - MARGIN;
	}
	else if(_Qubp_11x1[1] > 0.)
	{
		if((wstmax > _Qub_window.maxWST) && (_Qub_11x1[1] <_Qub_window.maxWST))
			wstmax = _Qub_window.maxWST + MARGIN;
	}

	if(_Qubp_11x1[2] < 0.)
	{
		if((rspmin < _Qub_window.minRSP) && (_Qub_11x1[2] >_Qub_window.minRSP))
			rspmin = _Qub_window.minRSP - MARGIN;
	}
	else if(_Qubp_11x1[2] > 0.)
	{
		if((rspmax > _Qub_window.maxRSP) && (_Qub_11x1[2] <_Qub_window.maxRSP))
			rspmax = _Qub_window.maxRSP + MARGIN;
	}

	if(_Qubp_11x1[3] < 0.)
	{
		if((rsrmin < _Qub_window.minRSR) && (_Qub_11x1[3] >_Qub_window.minRSR))
			rsrmin = _Qub_window.minRSR - MARGIN;
	}
	else if(_Qubp_11x1[3] > 0.)
	{
		if((rsrmax > _Qub_window.maxRSR) && (_Qub_11x1[3] <_Qub_window.maxRSR))
			rsrmax = _Qub_window.maxRSR + MARGIN;
	}

	if(_Qubp_11x1[4] < 0.)
	{
		if((rsymin < _Qub_window.minRSY) && (_Qub_11x1[4] >_Qub_window.minRSY))
			rsymin = _Qub_window.minRSY - MARGIN;
	}
	else if(_Qubp_11x1[4] > 0.)
	{
		if((rsymax > _Qub_window.maxRSY) && (_Qub_11x1[4] <_Qub_window.maxRSY))
			rsymax = _Qub_window.maxRSY + MARGIN;
	}

	if(_Qubp_11x1[5] < 0.)
	{
		if((rebmin < _Qub_window.minREB) && (_Qub_11x1[5] >_Qub_window.minREB))
			rebmin = _Qub_window.minREB - MARGIN;
	}
	else if(_Qubp_11x1[5] > 0.)
	{
		if((rebmax > _Qub_window.maxREB) && (_Qub_11x1[5] <_Qub_window.maxREB))
			rebmax = _Qub_window.maxREB + MARGIN;
	}

	if(_Qubp_11x1[6] < 0.)
	{
		if((rwymin < _Qub_window.minRWY) && (_Qub_11x1[6] >_Qub_window.minRWY))
			rwymin = _Qub_window.minRWY - MARGIN;
	}
	else if(_Qubp_11x1[6] > 0.)
	{
		if((rwymax > _Qub_window.maxRWY) && (_Qub_11x1[6] <_Qub_window.maxRWY))
	...
	if(_Qubp_11x1[7] < 0.)
	{
		if((lspmin < _Qub_window.minLSP) && (_Qub_11x1[7] >_Qub_window.minLSP))
			lspmin = _Qub_window.minLSP - MARGIN;
	}
	else if(_Qubp_11x1[7] > 0.)
	{
		if((lspmax > _Qub_window.maxLSP) && (_Qub_11x1[7] <_Qub_window.maxLSP))
			lspmax = _Qub_window.maxLSP + MARGIN;
	}

	if(_Qubp_11x1[8] < 0.)
	{
		if((lsrmin < _Qub_window.minLSR) && (_Qub_11x1[8] >_Qub_window.minLSR))
			lsrmin = _Qub_window.minLSR - MARGIN;
	}
	else if(_Qubp_11x1[8] > 0.)
	{
		if((lsrmax > _Qub_window.maxLSR) && (_Qub_11x1[8] <_Qub_window.maxLSR))
			lsrmax = _Qub_window.maxLSR + MARGIN;
	}

	if(_Qubp_11x1[9] < 0.)
	{
		if((lsymin < _Qub_window.minLSY) && (_Qub_11x1[9] >_Qub_window.minLSY))
			lsymin = _Qub_window.minLSY - MARGIN;
	}
	else if(_Qubp_11x1[9] > 0.)
	{
		if((lsymax > _Qub_window.maxLSY) && (_Qub_11x1[9] <_Qub_window.maxLSY))
			lsymax = _Qub_window.maxLSY + MARGIN;
	}

	if(_Qubp_11x1[10] < 0.)
	{
		if((lebmin < _Qub_window.minLEB) && (_Qub_11x1[10] >_Qub_window.minLEB))
			lebmin = _Qub_window.minLEB - MARGIN;
	}
	else if(_Qubp_11x1[10] > 0.)
	{
		if((lebmax > _Qub_window.maxLEB) && (_Qub_11x1[10] <_Qub_window.maxLEB))
			lebmax = _Qub_window.maxLEB + MARGIN;
	}

	if(_Qubp_11x1[11] < 0.)
	{
		if((lwymin < _Qub_window.minLWY) && (_Qub_11x1[11] >_Qub_window.minLWY))
			lwymin = _Qub_window.minLWY - MARGIN;
	}
	else if(_Qubp_11x1[11] > 0.)
	{
		if((lwymax > _Qub_window.maxLWY) && (_Qub_11x1[11] <_Qub_window.maxLWY))
	...
	if(_RWPp < 0.)
	{
		if((rwpmin < _Qub_window.minRWP) && (_RWP >_Qub_window.minRWP))
			rwpmin = _Qub_window.minRWP - MARGIN;
	}
	else if(_RWPp > 0.)
	{
		if((rwpmax > _Qub_window.maxRWP) && (_RWP <_Qub_window.maxRWP))
			rwpmax = _Qub_window.maxRWP + MARGIN;
	}

	if(_LWPp < 0.)
	{
		if((lwpmin < _Qub_window.minLWP) && (_LWP >_Qub_window.minLWP))
			lwpmin = _Qub_window.minLWP - MARGIN;
	}
	else if(_LWPp > 0.)
	{
		if((lwpmax > _Qub_window.maxLWP) && (_LWP <_Qub_window.maxLWP))
...
int MocapOnestep2(void)		// using preview window
{
	unsigned int i, j;
	...
	float des_qPEL_4x1[5], des_wPEL_3x1[4];
	float des_PCz, des_PCzp, des_PCzpp;
	...
	unsigned int tail_up_LWY, tail_down_LWY;
	unsigned int tail_up_LWP, tail_down_LWP;
	unsigned int tail_up_qPEL, tail_down_qPEL;
	unsigned int tail_up_PCz, tail_down_PCz;
	...
	if(_Qub_window.tail_qPEL == 0)
	{
		tail_up_qPEL = 1;
		tail_down_qPEL = WIND_SIZE_QPEL-1;
	}
	else 
	{
		tail_up_qPEL = (_Qub_window.tail_qPEL+1)%WIND_SIZE_QPEL;
		tail_down_qPEL = _Qub_window.tail_qPEL-1;
	}

	if(_Qub_window.tail_PCz == 0)
	{
		tail_up_PCz = 1;
		tail_down_PCz = WIND_SIZE_PCZ-1;
	}
	else 
	{
		tail_up_PCz = (_Qub_window.tail_PCz+1)%WIND_SIZE_PCZ;
		tail_down_PCz = _Qub_window.tail_PCz-1;
	}
	...
	des_qPEL_4x1[1] = _Qub_window.wind_qPEL[_Qub_window.tail_qPEL][1];
	des_qPEL_4x1[2] = _Qub_window.wind_qPEL[_Qub_window.tail_qPEL][2];
	des_qPEL_4x1[3] = _Qub_window.wind_qPEL[_Qub_window.tail_qPEL][3];
	des_qPEL_4x1[4] = _Qub_window.wind_qPEL[_Qub_window.tail_qPEL][4];
	des_PCz = _Qub_window.wind_PCz[_Qub_window.tail_PCz];
	...
	derive3(_Qub_window.wind_qPEL[tail_down_qPEL][1], des_qPEL_4x1[1], _Qub_window.wind_qPEL[tail_up_qPEL][1], &temp1_6x1[1], &temp2_6x1[1], DT);
	derive3(_Qub_window.wind_qPEL[tail_down_qPEL][2], des_qPEL_4x1[2], _Qub_window.wind_qPEL[tail_up_qPEL][2], &temp1_6x1[2], &temp2_6x1[2], DT);
	derive3(_Qub_window.wind_qPEL[tail_down_qPEL][3], des_qPEL_4x1[3], _Qub_window.wind_qPEL[tail_up_qPEL][3], &temp1_6x1[3], &temp2_6x1[3], DT);
	derive3(_Qub_window.wind_qPEL[tail_down_qPEL][4], des_qPEL_4x1[4], _Qub_window.wind_qPEL[tail_up_qPEL][4], &temp1_6x1[4], &temp2_6x1[4], DT);
	des_wPEL_3x1[1] = 2.f*(-des_qPEL_4x1[2]*temp1_6x1[1] + des_qPEL_4x1[1]*temp1_6x1[2] - des_qPEL_4x1[4]*temp1_6x1[3] + des_qPEL_4x1[3]*temp1_6x1[4]);
	des_wPEL_3x1[2] = 2.f*(-des_qPEL_4x1[3]*temp1_6x1[1] + des_qPEL_4x1[4]*temp1_6x1[2] + des_qPEL_4x1[1]*temp1_6x1[3] - des_qPEL_4x1[2]*temp1_6x1[4]);
	des_wPEL_3x1[3] = 2.f*(-des_qPEL_4x1[4]*temp1_6x1[1] - des_qPEL_4x1[3]*temp1_6x1[2] + des_qPEL_4x1[2]*temp1_6x1[3] + des_qPEL_4x1[1]*temp1_6x1[4]);

	derive3(_Qub_window.wind_PCz[tail_down_PCz], des_PCz, _Qub_window.wind_PCz[tail_up_PCz], &des_PCzp, &des_PCzpp, DT);
	derive3(_Qub_window.wind_WST[tail_down_WST], des_WST, _Qub_window.wind_WST[tail_up_WST], &des_WSTp, &des_WSTpp, DT);
	derive3(_Qub_window.wind_RSP[tail_down_RSP], des_RSP, _Qub_window.wind_RSP[tail_up_RSP], &des_RSPp, &des_RSPpp, DT);
	derive3(_Qub_window.wind_RSR[tail_down_RSR], des_RSR, _Qub_window.wind_RSR[tail_up_RSR], &des_RSRp, &des_RSRpp, DT);
	derive3(_Qub_window.wind_RSY[tail_down_RSY], des_RSY, _Qub_window.wind_RSY[tail_up_RSY], &des_RSYp, &des_RSYpp, DT);
	...
	_Qub_window.tail_qPEL = (_Qub_window.tail_qPEL+1)%WIND_SIZE_QPEL;
	_Qub_window.tail_PCz = (_Qub_window.tail_PCz+1)%WIND_SIZE_PCZ;
	_Qub_window.tail_WST = (_Qub_window.tail_WST+1)%WIND_SIZE_WST;
	_Qub_window.tail_RSP = (_Qub_window.tail_RSP+1)%WIND_SIZE_RSP;
	_Qub_window.tail_RSR = (_Qub_window.tail_RSR+1)%WIND_SIZE_RSR;
	_Qub_window.tail_RSY = (_Qub_window.tail_RSY+1)%WIND_SIZE_RSY;
	...
	//--------- PEL
	QTdel(des_qPEL_4x1, &_Qlb_19x1[3], temp3_3x1);
	Xpel_3x1[1] = KV_PEL*(des_wPEL_3x1[1]-_Qlbp_18x1[4]) + KP_PEL*temp3_3x1[1];
	Xpel_3x1[2] = KV_PEL*(des_wPEL_3x1[2]-_Qlbp_18x1[5]) + KP_PEL*temp3_3x1[2];
	Xpel_3x1[3] = KV_PEL*(des_wPEL_3x1[3]-_Qlbp_18x1[6]) + KP_PEL*temp3_3x1[3];

	//-----------------

	//--------- PCz
	Xpcz_1x1 = des_PCzpp + KV_PCZ*(des_PCzp-_Qlbp_18x1[3]) + KP_PCZ*(des_PCz-_Qlb_19x1[3]);
	//-----------------
...
int InitQubWindow(float qPEL0, float qPELx, float qPELy, float qPELz, float PCz, 
					 float wst, float rsp, float rsr, float rsy, float reb, float rwy, float rwp,
				     float lsp, float lsr, float lsy, float leb, float lwy, float lwp,
					 float dNKY, float dNK1, float dNK2,
					 float dRF1, float dRF2, float dRF3, float dRF4, float dRF5,
					 ...
					 float body_pitch)
{
	...
	for(i=0; i<WIND_SIZE_QPEL; i++)			
	{
		_Qub_window.wind_qPEL[i][1] = qPEL0;
		_Qub_window.wind_qPEL[i][2] = qPELx;
		_Qub_window.wind_qPEL[i][3] = qPELy;
		_Qub_window.wind_qPEL[i][4] = qPELz;
	}
	for(i=0; i<WIND_SIZE_PCZ; i++)			
		_Qub_window.wind_PCz[i] = PCz;
	...
	_Qub_window.head_qPEL = 0;
	_Qub_window.head_PCz = 0;
	...
	_Qub_window.tail_qPEL = 4;
	_Qub_window.tail_PCz = 4;
...
int Back2InitialPos(void)
{
	float rhypp = -KV_MOCAP*_Qlbp_18x1[7] + KP_MOCAP*(_RHY0-_Qlb_19x1[8]);
	float rhrpp = -KV_MOCAP*_Qlbp_18x1[8] + KP_MOCAP*(_RHR0-_Qlb_19x1[9]);
	float rhppp = -KV_MOCAP*_Qlbp_18x1[9] + KP_MOCAP*(_RHP0-_Qlb_19x1[10]);
	float rknpp = -KV_MOCAP*_Qlbp_18x1[10] + KP_MOCAP*(_RKN0-_Qlb_19x1[11]);
	float rappp = -KV_MOCAP*_Qlbp_18x1[11] + KP_MOCAP*(_RAP0-_Qlb_19x1[12]);
	float rarpp = -KV_MOCAP*_Qlbp_18x1[12] + KP_MOCAP*(_RAR0-_Qlb_19x1[13]);

	float lhypp = -KV_MOCAP*_Qlbp_18x1[13] + KP_MOCAP*(_LHY0-_Qlb_19x1[14]);
	float lhrpp = -KV_MOCAP*_Qlbp_18x1[14] + KP_MOCAP*(_LHR0-_Qlb_19x1[15]);
	float lhppp = -KV_MOCAP*_Qlbp_18x1[15] + KP_MOCAP*(_LHP0-_Qlb_19x1[16]);
	float lknpp = -KV_MOCAP*_Qlbp_18x1[16] + KP_MOCAP*(_LKN0-_Qlb_19x1[17]);
	float lappp = -KV_MOCAP*_Qlbp_18x1[17] + KP_MOCAP*(_LAP0-_Qlb_19x1[18]);
	float larpp = -KV_MOCAP*_Qlbp_18x1[18] + KP_MOCAP*(_LAR0-_Qlb_19x1[19]);

	float Qubpp_11x1[12], RWPpp, LWPpp;
	
	JAVLC(_Qub_11x1[1], _Qubp_11x1[1],  KV_MOCAP*(-_Qubp_11x1[1])+KP_MOCAP*(_WST0-_Qub_11x1[1]),   WSTpmax, WSTppmax, WSTmin, WSTmax, MARGIN, &Qubpp_11x1[1]);
	JAVLC(_Qub_11x1[2], _Qubp_11x1[2],  KV_MOCAP*(-_Qubp_11x1[2])+KP_MOCAP*(_RSP0-_Qub_11x1[2]),   RSPpmax, RSPppmax, RSPmin, RSPmax, MARGIN, &Qubpp_11x1[2]);
	JAVLC(_Qub_11x1[3], _Qubp_11x1[3],  KV_MOCAP*(-_Qubp_11x1[3])+KP_MOCAP*(_RSR0-_Qub_11x1[3]),   RSRpmax, RSRppmax, RSRmin, RSRmax, MARGIN, &Qubpp_11x1[3]);
	JAVLC(_Qub_11x1[4], _Qubp_11x1[4],  KV_MOCAP*(-_Qubp_11x1[4])+KP_MOCAP*(_RSY0-_Qub_11x1[4]),   RSYpmax, RSYppmax, RSYmin, RSYmax, MARGIN, &Qubpp_11x1[4]);
	JAVLC(_Qub_11x1[5], _Qubp_11x1[5],  KV_MOCAP*(-_Qubp_11x1[5])+KP_MOCAP*(_REB0-_Qub_11x1[5]),   REPpmax, REPppmax, REPmin, REPmax, MARGIN, &Qubpp_11x1[5]);
	JAVLC(_Qub_11x1[6], _Qubp_11x1[6],  KV_MOCAP*(-_Qubp_11x1[6])+KP_MOCAP*(_RWY0-_Qub_11x1[6]),   REYpmax, REYppmax, REYmin, REYmax, MARGIN, &Qubpp_11x1[6]);
	JAVLC(_Qub_11x1[7], _Qubp_11x1[7],  KV_MOCAP*(-_Qubp_11x1[7])+KP_MOCAP*(_LSP0-_Qub_11x1[7]),   LSPpmax, LSPppmax, LSPmin, LSPmax, MARGIN, &Qubpp_11x1[7]);
	JAVLC(_Qub_11x1[8], _Qubp_11x1[8],  KV_MOCAP*(-_Qubp_11x1[8])+KP_MOCAP*(_LSR0-_Qub_11x1[8]),   LSRpmax, LSRppmax, LSRmin, LSRmax, MARGIN, &Qubpp_11x1[8]);
	JAVLC(_Qub_11x1[9], _Qubp_11x1[9],  KV_MOCAP*(-_Qubp_11x1[9])+KP_MOCAP*(_LSY0-_Qub_11x1[9]),   LSYpmax, LSYppmax, LSYmin, LSYmax, MARGIN, &Qubpp_11x1[9]);
	JAVLC(_Qub_11x1[10],_Qubp_11x1[10], KV_MOCAP*(-_Qubp_11x1[10])+KP_MOCAP*(_LEB0-_Qub_11x1[10]), LEPpmax, LEPppmax, LEPmin, LEPmax, MARGIN, &Qubpp_11x1[10]);
	JAVLC(_Qub_11x1[11],_Qubp_11x1[11], KV_MOCAP*(-_Qubp_11x1[11])+KP_MOCAP*(_LWY0-_Qub_11x1[11]), LEYpmax, LEYppmax, LEYmin, LEYmax, MARGIN, &Qubpp_11x1[11]);

	JAVLC(_RWP,_RWPp, KV_MOCAP*(-_RWPp)+KP_MOCAP*(_RWP0-_RWP), RWPpmax, RWPppmax, RWPmin, RWPmax, MARGIN, &RWPpp);
	JAVLC(_LWP,_LWPp, KV_MOCAP*(-_LWPp)+KP_MOCAP*(_LWP0-_LWP), LWPpmax, LWPppmax, LWPmin, LWPmax, MARGIN, &LWPpp);

	JAVLC(_Qlb_19x1[8], _Qlbp_18x1[7], rhypp, RHYpmax, RHYppmax, RHYmin, RHYmax, D2R, &rhypp);
	JAVLC(_Qlb_19x1[9], _Qlbp_18x1[8], rhrpp, RHRpmax, RHRppmax, RHRmin, RHRmax, D2R, &rhrpp);		
	JAVLC(_Qlb_19x1[10], _Qlbp_18x1[9], rhppp, RHPpmax, RHPppmax, RHPmin, RHPmax, D2R, &rhppp);		
	JAVLC(_Qlb_19x1[11], _Qlbp_18x1[10], rknpp, RKNpmax, RKNppmax, RKNmin, RKNmax, D2R, &rknpp);		
	JAVLC(_Qlb_19x1[12], _Qlbp_18x1[11], rappp, RAPpmax, RAPppmax, RAPmin, RAPmax, D2R, &rappp);		
	JAVLC(_Qlb_19x1[13], _Qlbp_18x1[12], rarpp, RARpmax, RARppmax, RARmin, RARmax, D2R, &rarpp);		
	JAVLC(_Qlb_19x1[14], _Qlbp_18x1[13], lhypp, LHYpmax, LHYppmax, LHYmin, LHYmax, D2R, &lhypp);		
	JAVLC(_Qlb_19x1[15], _Qlbp_18x1[14], lhrpp, LHRpmax, LHRppmax, LHRmin, LHRmax, D2R, &lhrpp);		
	JAVLC(_Qlb_19x1[16], _Qlbp_18x1[15], lhppp, LHPpmax, LHPppmax, LHPmin, LHPmax, D2R, &lhppp);		
	JAVLC(_Qlb_19x1[17], _Qlbp_18x1[16], lknpp, LKNpmax, LKNppmax, LKNmin, LKNmax, D2R, &lknpp);		
	JAVLC(_Qlb_19x1[18], _Qlbp_18x1[17], lappp, LAPpmax, LAPppmax, LAPmin, LAPmax, D2R, &lappp);		
	JAVLC(_Qlb_19x1[19], _Qlbp_18x1[18], larpp, LARpmax, LARppmax, LARmin, LARmax, D2R, &larpp);

	_Qlb_19x1[8] += _Qlbp_18x1[7]*DT + 0.5f*rhypp*DT*DT;
	_Qlb_19x1[9] += _Qlbp_18x1[8]*DT + 0.5f*rhrpp*DT*DT;
	_Qlb_19x1[10] += _Qlbp_18x1[9]*DT + 0.5f*rhppp*DT*DT;
	_Qlb_19x1[11] += _Qlbp_18x1[10]*DT + 0.5f*rknpp*DT*DT;
	_Qlb_19x1[12] += _Qlbp_18x1[11]*DT + 0.5f*rappp*DT*DT;
	_Qlb_19x1[13] += _Qlbp_18x1[12]*DT + 0.5f*rarpp*DT*DT;
	_Qlb_19x1[14] += _Qlbp_18x1[13]*DT + 0.5f*lhypp*DT*DT;
	_Qlb_19x1[15] += _Qlbp_18x1[14]*DT + 0.5f*lhrpp*DT*DT;
	_Qlb_19x1[16] += _Qlbp_18x1[15]*DT + 0.5f*lhppp*DT*DT;
	_Qlb_19x1[17] += _Qlbp_18x1[16]*DT + 0.5f*lknpp*DT*DT;
	_Qlb_19x1[18] += _Qlbp_18x1[17]*DT + 0.5f*lappp*DT*DT;
	_Qlb_19x1[19] += _Qlbp_18x1[18]*DT + 0.5f*larpp*DT*DT;
	
	_Qlbp_18x1[7] += DT*rhypp;
	_Qlbp_18x1[8] += DT*rhrpp;
	_Qlbp_18x1[9] += DT*rhppp;
	_Qlbp_18x1[10] += DT*rknpp;
	_Qlbp_18x1[11] += DT*rappp;
	_Qlbp_18x1[12] += DT*rarpp;
	_Qlbp_18x1[13] += DT*lhypp;
	_Qlbp_18x1[14] += DT*lhrpp;
	_Qlbp_18x1[15] += DT*lhppp;
	_Qlbp_18x1[16] += DT*lknpp;
	_Qlbp_18x1[17] += DT*lappp;
	_Qlbp_18x1[18] += DT*larpp;
